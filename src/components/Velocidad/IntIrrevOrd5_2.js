...
export const IntIrrevOrd5_2 = () => {
  ...
  const calculate = (data) => {
    ...
    for (let i = 1; i <= index; i++) {
      ...
      // Calculate k
      var k = new Number();
      k = ( 1 / (parseFloat(data['ca_'+i]) * Math.sqrt(parseFloat(data['ca_'+i])) ) ) - ( 1 / (parseFloat(data['ca_0']) * Math.sqrt(parseFloat(data['ca_0'])) ) );
      k = k * ( 2 / (3 * parseFloat(data['t_'+i]) ) );
      //console.log(k)
    ...
    <div className="card">
      <div className="card-body">
        <h5 className="card-title">
          Método Integral para una reacción no reversible de orden 5/2
        </h5>
              ...
              <table className="table table-striped">
                <thead>
                  <tr>
                    ...
                    <th scope="col">t(s)</th>
                    <th scope="col">K (L 3/2 / (seg mol 5/2))</th>
                  </tr>
                </thead>
                <tbody>
                  {rowsSize.map((row, i) => {
