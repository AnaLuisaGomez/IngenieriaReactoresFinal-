...
export const IntIrrevOrd3 = () => {
  ...
  const calculate = (data) => {
    ...
    for (let i = 1; i <= index; i++) {
      ...
      // Calculate k
      var k = new Number();
      k =  (1 / Math.pow( parseFloat(data['ca_'+i] ), 2) ) - (1 / Math.pow( parseFloat(data['ca_0'] ), 2) );
      k = k / ( 2 * parseFloat(data['t_'+i]));
      //console.log(k)
    ...
    <div className="card">
      <div className="card-body">
        <h5 className="card-title">
          Método Integral para una reacción no reversible de orden 3
        </h5>
              ...
              <table className="table table-striped">
                <thead>
                  <tr>
                    ...
                    <th scope="col">t(s)</th>
                    <th scope="col">K (L2 / (mol^2 * seg) )</th>
                  </tr>
                </thead>
                <tbody>
                  {rowsSize.map((row, i) => {
