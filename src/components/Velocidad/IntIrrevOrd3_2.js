...
export const IntIrrevOrd3_2 = () => {
  ...
  const calculate = (data) => {
    ...
    for (let i = 1; i <= index; i++) {
      ...
      // Calculate k
      var k = new Number();
      k = ( 2 / (Math.pow(parseFloat(data["ca_"+i]), 0.5) * parseFloat(data['t_'+i]) ) ) - ( 2 / (Math.pow(parseFloat(data["ca_0"]), 0.5)  * parseFloat(data['t_'+i])) );
    ...
    <div className="card">
      <div className="card-body">
        <h5 className="card-title">
          Método Integral para una reacción no reversible de orden 3/2
        </h5>
              ...
              <table className="table table-striped">
                <thead>
                  <tr>
                    ...
                    <th scope="col">t(s)</th>
                    <th scope="col">K (L1/2 / (mol1/2 * seg))</th>
                  </tr>
                </thead>
                <tbody>
                  {rowsSize.map((row, i) => {
