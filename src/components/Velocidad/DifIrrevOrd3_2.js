...
export const DifIrrevOrd3_2 = () => {
  ...
  const calculate = (data) => {
    ...
    for (let i = 1; i <= index; i++) {
      ...
      k = (k / t) * -1;
      k = k / (( Math.pow(parseFloat(data["ca_" + i]), 1.5) + Math.pow(parseFloat(data["ca_" + j]), 1.5))  / 2);
      //console.log(k)
      document.getElementById("k_" + i).value = k;
      document.getElementById("t_" + index).value = t;
    }
    ...
    <div className="card">
      <div className="card-body">
        <h5 className="card-title">
          Método diferencial para una reacción no reversible de orden 3/2
        </h5>
